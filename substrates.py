...
#            self.mcds_field_cb()
        ...
        self.show_grid = Checkbox(
            description='Grid',
            disabled=False,
#           layout=Layout(width=constWidth2),
        )
        # self.show_grid.observe(show_grid_cb)

        #---------------
        field_cmap_row2 = HBox([self.field_cmap, self.cmap_fixed, self.show_grid])
...
#        self.tab = HBox([mcds_params, self.mcds_plot])
        ...
        row2 = Box([self.cmap_fixed, self.cmap_min, self.cmap_max, self.show_grid], layout=Layout(border='0px solid black',
                            width='80%',
                            height='',
                            align_items='stretch',
                            flex_direction='row',
...
#            print("No: ", full_fname)
            print("Once file is generated, click/drag slider")  # No:  output00000000_microenvironment0.mat
...
#        scipy.io.loadmat(fullname, info_dict)
        ...
        if (True):   # need to parse vals from the config.xml
        ...
        if (self.show_grid.value):
            xs = np.linspace(xmin,xmax,self.numx)
            ys = np.linspace(ymin,ymax,self.numy)
